...
// функция проведения атаки героем
func attack(charName, charClass string) string {
	if charClass == "warrior" {
		return fmt.Sprintf("%s нанес урон противнику равный %d.", charName, 5+makeRandomVariable(3, 5))
	}

	if charClass == "mage" {
		return fmt.Sprintf("%s нанес урон противнику равный %d.", charName, 5+makeRandomVariable(5, 10))
	}

	if charClass == "healer" {
		return fmt.Sprintf("%s нанес урон противнику равный %d.", charName, 5+makeRandomVariable(-3, -1))
	}
	return "неизвестный класс персонажа"
}

// функция проведения защиты героем
func defence(charName, charClass string) string {
	if charClass == "warrior" {
		return fmt.Sprintf("%s блокировал %d урона.", charName, 10+makeRandomVariable(5, 10))
	}

	if charClass == "mage" {
		return fmt.Sprintf("%s блокировал %d урона.", charName, 10+makeRandomVariable(-2, 2))
	}

	if charClass == "healer" {
		return fmt.Sprintf("%s блокировал %d урона.", charName, 10+makeRandomVariable(2, 5))
	}

	return "неизвестный класс персонажа"
}

// функция выполнения специального приема героем
...
// функция выполнения тренировки героем
...
// функция рандомизации значений
func makeRandomVariable(min, max int) int {
